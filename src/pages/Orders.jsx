...
import { getSellerOrders, updateOrderStatus } from '../api/orderApi'; // endpoints #3 and #5
...
const STATUS_ICONS = { Pending: '⏳', Confirmed: '✅', Delivered: '🏁', Cancelled: '❌' };
...
export default function Orders() {
  ...
  // Track which order IDs are currently being updated (to disable their dropdowns)
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      // GET /api/orders/seller/orders — fetch all orders for this vendor's products
  ...
  // PUT /api/orders/:id/status — vendor updates the status of one order
                ...
                filtered.map((order) => {
                  ...
                  return (
                    <tr key={orderId} className="table-row-hover">
                      ...
                      {/* Status — editable dropdown (PUT /api/orders/:id/status) */}
                      <td className="px-5 py-4">
                        <select
                          ...
                          className={`input-field text-xs py-1 px-2 w-auto rounded-lg
                            ...
                            ${order.status === 'Confirmed' ? 'border-blue-300'    : ''}
                            ${order.status === 'Delivered' ? 'border-emerald-300' : ''}
